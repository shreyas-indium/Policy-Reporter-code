...
def generate_cat(input_excel, start_rng, end_rng):

    last_catagory = ''
    last_subcategory = ''


    output_excel = re.sub('.xlsx','_out.xlsx',input_excel)
    writer = pd.ExcelWriter(output_excel, engine='xlsxwriter')
    for j in range(start_rng, end_rng):

        print('j',j)

        for page in [1,2]:

            df1 = pd.read_excel(input_excel,sheet_name='page'+str(j)+'_table_'+str(page), engine='openpyxl')
            if len(df1) > 0:
                data_to_analyse = df1
                category_value = ''
                subcategory_value = ''
                ...
                #last_catagory = data_to_analyse.iloc[0].values[0]
                #last_subcategory = data_to_analyse.iloc[1].values[0]

                #data_to_analyse = data_to_analyse.drop([0,1,2],axis=0)

                #data_to_analyse.columns = ['Drug Name','Tier','Notes']
                ...
                data_to_analyse.to_excel(writer, sheet_name='Table' + str(j)+'_'+str(page), encoding='utf-8')
    writer.save()
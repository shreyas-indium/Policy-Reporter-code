...
def extract_data(inputpdf,outputxl, page_range):
    """
    ...
    last_catagory = ''
    last_subcategory = ''
    writer = pd.ExcelWriter(outputxl, engine='xlsxwriter')
    for j in range(page_range[0],page_range[1]):
        df1 = read_pdf(inputpdf,pages=j,multiple_tables=False)
        if len(df1)>0:
            print("df1 ", df1)
            data_to_analyse = df1[0]
            category_value = ''
            subcategory_value = ''
            category = []
            subcategory = []
            for i in range(0,data_to_analyse.shape[0]):
                ...
                if i==0:
                    ...
                    if (boolean.iloc[0].values[1] ==True) & (boolean.iloc[0].values[2]== True):
                        ...
                        if (initial_category_check.iloc[0].values[1] ==False) or (initial_category_check.iloc[0].values[2]== False):
                            category_value = last_catagory
                ...
                if len(boolean.loc[(boolean[boolean.columns[1]] == True) & (boolean[boolean.columns[2]] == True)])>0:
                    ...
                    if len(category_check.loc[(category_check[category_check.columns[1]] == True) & (category_check[category_check.columns[2]] == True)])>0:
            ...
            df_bool = boolean_drop.loc[(boolean_drop[boolean_drop.columns[1]] == True) & (boolean_drop[boolean_drop.columns[2]] == True)]
